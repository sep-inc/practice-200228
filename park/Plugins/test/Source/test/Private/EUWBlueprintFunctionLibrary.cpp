...
FArchive* CreatePakWriter(const TCHAR* Filename, const FKeyChain& InKeyChain, bool bSign)
{
	...
	if (Writer)
	{
		if (bSign)
		{
			...
			//FSignedArchiveWriter test;
			//return  FSignedArchiveWriter(*Writer, Filename, InKeyChain.SigningKey);
		}
		else
		{
...
void UEUWBlueprintFunctionLibrary::CreatePackage2(UObject* obj, FString file_name) {
	...
	FString inname = FString("C:/git3/park/Content/Mod/MyProject-WindowsNoEditor.pak");
	const FString outname = FString("C:/git3/park/Content/Gun");
	...
	/*FPakFile* Pak = new FPakFile(LocalPlatformFile, *inname, false);
	...
	PakPlatform->Mount(*inname, PakOrder,*outname);
