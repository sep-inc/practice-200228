...
void UMODToolEditorUtilityWidgetCPP::CreateLocalMod(FString mod_name) {
	...
	for (int i = 0; i < validity_parames.Num(); i++) {
		...
		CreateLocalWaveMod(mod_name);
		
		}
	}
}

void UMODToolEditorUtilityWidgetCPP::CreateLocalWaveMod(FString mod_name) {
	...
	for (int map_index = 0; map_index < map_quest_param.Num(); map_index++) {
		...
		}
		{
			...
			FuncLib::SetJsonFieldVal_String(quest_JsonObject, TEXT("Wave"), map_quest_param[map_index].other_list.WaveName);
		...
		}
		{
			...
			for (int wave_index = 0; wave_index < map_quest_param[map_index].wave.Num(); wave_index++) {
				...
				TSharedPtr<FJsonValueObject> obj = MakeShareable(new FJsonValueObject(wave_JsonObject));
				wave_JsonArrayObject.Add(obj);
			}
			...
			UMODToolBlueprintFunctionLibrary::FileCreate(mod_name + ("/Wave/") + map_quest_param[map_index].other_list.WaveName, OutputString);
		}
	}
